...
class RM3100_timed():
        ...
        def spi(self):
                ...
                while time.time() < t_end:
                        ...
                        if (time.time() - trigger) < len(raw)*(1/rate):
                                # wait until appropriate time
                                time.sleep(len(raw)*(1/rate) + trigger - time.time())
                        else:
                                print('Timing error.')
                                return
                ...
                np.savetxt('mag_data_short.txt', mag_data, header=sensor_settings1+' ; '+sensor_settings2)
